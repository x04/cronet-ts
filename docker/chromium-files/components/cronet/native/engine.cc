...
Cronet_RESULT Cronet_EngineImpl::StartWithParams(
  ...
  // Parse proxy configuration from experimental_options JSON.
  // Format: {"proxy": {"url": "http://host:port"}} or
  //         {"proxy": {"proxies": [{"scheme": "http", "host": "h", "port": 80}]}}
  if (!params->experimental_options.empty()) {
    auto parsed = base::JSONReader::Read(params->experimental_options,
                                           base::JSON_PARSE_CHROMIUM_EXTENSIONS);
    if (parsed) {
      const base::DictValue* root_dict = parsed->GetIfDict();
      if (root_dict) {
        const base::DictValue* proxy_dict = root_dict->FindDict("proxy");
        if (proxy_dict) {
          cronet::proto::ProxyOptions proxy_options;

          // Simple URL format: {"proxy": {"url": "http://host:port"}}
          const std::string* proxy_url = proxy_dict->FindString("url");
          if (proxy_url && !proxy_url->empty()) {
            ...
            if (url.is_valid()) {
              ...
              proxy->set_host(url.host());
              proxy->set_port(url.EffectiveIntPort());
              context_config_builder.proxy_options = std::move(proxy_options);
            }
          }

          // Array format: {"proxy": {"proxies": [...]}}
          const base::ListValue* proxies_list =
              proxy_dict->FindList("proxies");
          if (proxies_list) {
            for (size_t i = 0; i < proxies_list->size(); ++i) {
              const base::Value& entry = (*proxies_list)[i];
              const base::DictValue* p = entry.GetIfDict();
              if (!p) continue;
              auto* proxy = proxy_options.add_proxies();

              const std::string* scheme = p->FindString("scheme");
              if (scheme && *scheme == "https") {
                proxy->set_scheme(cronet::proto::HTTPS);
              } else if (scheme && *scheme == "direct") {
                proxy->set_scheme(cronet::proto::DIRECT);
              } else {
                proxy->set_scheme(cronet::proto::HTTP);
              }

              const std::string* host = p->FindString("host");
              if (host) proxy->set_host(*host);

              std::optional<int> port = p->FindInt("port");
              if (port) proxy->set_port(*port);
            }
            if (proxy_options.proxies_size() > 0) {
              context_config_builder.proxy_options = std::move(proxy_options);
            }
          }
        }
